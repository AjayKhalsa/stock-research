...
function BacktestBreakdown({ eyebrow, title, rows }) {
  const available = (rows || []).filter(row => row.sample > 0);
  return <div className="cfo-panel"><SectionHeader eyebrow={eyebrow} title={title} />
    {available.length ? <dl className="cfo-definition">{available.slice(0, 10).map(row => <div key={row.group}><dt>{String(row.group).replaceAll('_', ' ')}</dt><dd>{row.sample} trades · {fmt(row.net_expectancy_r, 3)}R net expectancy</dd></div>)}</dl> : <p className="cfo-muted">No resolved trades in this breakdown yet.</p>}
  </div>;
}
...
function System({ job, brief }) {
  ...
  const legacyShadow = backtest.shadow_test || {};
  const promotion = legacyShadow.promotion_policy || {};
  const calibration = brief.shadow_model || {};
  ...
  const archive = brief.data_archive_status || {};
  const archiveCounts = archive.counts || {};
  const archiveAudit = archive.latest_audit || brief.data_archive?.quality_audit || {};
  const archiveAuditMetrics = archiveAudit.metrics || {};
