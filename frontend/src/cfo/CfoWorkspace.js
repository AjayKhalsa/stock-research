...
function PositionSizeCalculator({ plan, data, defaultRisk = 0.5 }) {
  const [portfolioValue, setPortfolioValue] = useState('');
  const [riskPct, setRiskPct] = useState(defaultRisk);
  const entry = plan?.entry?.low != null && plan?.entry?.high != null ? (Number(plan.entry.low) + Number(plan.entry.high)) / 2 : null;
  const stop = plan?.stop?.price == null ? null : Number(plan.stop.price);
  const riskPerShare = entry != null && stop != null ? entry - stop : null;
  const maxLoss = Number(portfolioValue) > 0 && Number(riskPct) > 0 ? Number(portfolioValue) * Number(riskPct) / 100 : null;
  const quantity = maxLoss != null && riskPerShare > 0 ? Math.floor(maxLoss / riskPerShare) : null;
  const [tracking, setTracking] = useState(false);
  const [trackingState, setTrackingState] = useState('');
  const trackPlan = async () => {
    const targets = plan?.targets || [];
    if (!data?.symbol || entry == null || stop == null || !targets[0]?.price) return;
    setTracking(true); setTrackingState('');
    try {
      await createPaperTrade({ symbol: data.symbol, entry_price: entry,
        entry_low: plan.entry.low, entry_high: plan.entry.high, stop_loss: stop,
        target_t1: targets[0].price, target_t2: targets[1]?.price || targets[0].price,
        score: data.score, setup_type: data.setup_type,
        signal_date: data.evidence?.price?.as_of, snapshot_id: data.snapshot_id,
        model_version: data.evidence?.model?.version, action_at_add: data.action,
        invalidation: plan.invalidation });
      setTrackingState('Armed. Daily candles will determine whether entry triggers before expiry.');
      window.dispatchEvent(new CustomEvent('stocklens:paper-trade-changed'));
    } catch (error) {
      setTrackingState(error?.response?.data?.detail || 'Could not arm this paper test.');
    } finally { setTracking(false); }
  };
  const canTrack = data?.symbol && riskPerShare > 0 && plan?.targets?.[0]?.price;
...
function CandidateDossier({ data, loading, error, symbol, onBack, onResearch, settings }) {
  ...
  const momentum = data.earnings_momentum || {};
  const momentumMetrics = momentum.metrics || {};
  const dataConfidence = data.data_confidence || { overall: data.confidence };
  ...
  return <div className="cfo-page cfo-dossier">
    ...
    <header className="cfo-decision-header"><div><span className="cfo-eyebrow">{rankText}</span><h1>{data.symbol} <small>{data.company}</small></h1><p>{data.action_reason || data.setup_label || 'No active setup'}</p></div><StatusPill value={data.action}>{data.classification || ACTION_LABEL[data.action]}</StatusPill><div className="cfo-decision-numbers"><Metric label="Entry" value={`${money(plan.entry?.low)}–${money(plan.entry?.high)}`} /><Metric label="Stop" value={money(plan.stop?.price)} /><Metric label="Reward / risk" value={`${fmt(plan.risk_reward, 2)}×`} /><Metric label="Data completeness" value={`${fmt(dataConfidence.overall, 0)}%`} note="not win probability" /></div></header>
    ...
    {tab === 'evidence' && <section className="cfo-tab-grid"><div className="cfo-panel"><SectionHeader eyebrow="Sources" title="How complete is this?" detail="These percentages describe evidence coverage, never the chance that a trade wins." /><dl className="cfo-definition"><div><dt>Price data</dt><dd>{fmt(dataConfidence.price_data, 0)}% · {data.evidence?.price?.status}</dd></div><div><dt>Financial data</dt><dd>{fmt(dataConfidence.financial_data, 0)}% · {data.evidence?.fundamentals?.origin || 'unavailable'}</dd></div><div><dt>Event data</dt><dd>{fmt(dataConfidence.event_data, 0)}%</dd></div><div><dt>AI extraction</dt><dd>{dataConfidence.ai_extraction == null ? 'Not run' : `${fmt(dataConfidence.ai_extraction, 0)}%`}</dd></div><div><dt>Rules version</dt><dd>{data.evidence?.model?.version}</dd></div></dl></div><div className="cfo-panel"><SectionHeader eyebrow="AI review" title="What AI can and cannot do" /><p>AI may explain or lower a status. It cannot change the numbers, promote a blocked stock, or pretend missing evidence is positive.</p><StatusPill value="neutral">{data.evidence?.ai_committee?.status?.replace('_', ' ') || 'Not run'}</StatusPill><button className="cfo-primary" onClick={() => onResearch(data.symbol)}>Open full live research</button></div><ExternalEvidence research={data.external_research} /></section>}
  </div>;
}
...
function Portfolio({ snapshot, settings, onSave }) {
  ...
  const openTrades = trades.filter(t => t.status === 'ARMED' || t.status === 'ACTIVE');
  const openCount = (snapshot?.stats?.active_count || 0) + (snapshot?.stats?.armed_count || 0);
  return <div className="cfo-page"><SectionHeader eyebrow="Portfolio" title="Your open trades" detail="See open positions, concentration and upcoming results before adding another stock." />
    <section className="cfo-summary-strip"><Metric label="Open tests" value={openCount} note={`${snapshot?.stats?.armed_count || 0} waiting · ${snapshot?.stats?.active_count || 0} active`} /><Metric label="Resolved paper trades" value={snapshot?.stats?.resolved_count ?? ((snapshot?.stats?.wins || 0) + (snapshot?.stats?.losses || 0))} note="confidence remains early to 100" /><Metric label="Win rate" value={`${fmt(snapshot?.stats?.win_rate_pct)}%`} /><Metric label="Expectancy / trade" value={`${fmt(snapshot?.stats?.expectancy_r, 2)}R`} /></section>
    <section className="cfo-two-col"><div className="cfo-panel"><SectionHeader eyebrow="Open tests" title="Paper tracking" />{openTrades.length ? openTrades.map(t => <div className="cfo-trade" key={t.id}><strong>{t.symbol}</strong><span>{t.status === 'ARMED' ? 'Zone' : 'Entry'} <b className="mono">{t.status === 'ARMED' && t.entry_low != null ? `${money(t.entry_low)}–${money(t.entry_high)}` : money(t.entry_price)}</b></span><span>Stop <b className="mono">{money(t.stop_loss)}</b></span><StatusPill value={t.status === 'ACTIVE' ? 'healthy' : 'neutral'}>{t.status === 'ACTIVE' ? 'Active' : 'Waiting for entry'}</StatusPill></div>) : <div className="cfo-quiet"><span>—</span><div><strong>No open paper tests</strong><p>Use paper tracking to build a real result history before relying on the rankings.</p></div></div>}</div>
...
function System({ job, brief }) {
  const pct = job?.total ? Math.round((job.progress || 0) / job.total * 100) : 0;
  const truth = brief.historical_truth || {};
  const backtest = brief.latest_backtest || {};
  const validation = backtest.overall || {};
  const shadow = backtest.shadow_test || {};
  const promotion = shadow.promotion_policy || {};
  ...
  return <div className="cfo-page"><SectionHeader eyebrow="System" title="Data, automation and audit" detail="The workspace prepares itself. Job controls stay here, away from daily decisions." />
    ...
    <section className="cfo-panel"><SectionHeader eyebrow="Historical truth" title="Automatic recommendation outcomes" detail="Every ready-now or near-entry recommendation is followed from its original snapshot. Observational rows help calibrate V2 but never enter the live scorecard." /><div className="cfo-summary-strip"><Metric label="Tracked" value={truth.total || 0} note={`${truth.armed || 0} waiting · ${truth.active || 0} active`} /><Metric label="Resolved" value={truth.resolved || 0} note={`${truth.excluded || 0} excluded`} /><Metric label="Observational" value={truth.observational || 0} note={`${truth.observational_resolved || 0} resolved`} /><Metric label="Win rate" value={`${fmt(truth.win_rate_pct)}%`} /><Metric label="Expectancy" value={`${fmt(truth.expectancy_r, 2)}R`} /><Metric label="Average MFE" value={`${fmt(truth.avg_mfe_r, 2)}R`} /><Metric label="Average MAE" value={`${fmt(truth.avg_mae_r, 2)}R`} /></div></section>
    <section className="cfo-panel"><SectionHeader eyebrow="Point-in-time validation" title="Cost-adjusted snapshot replay" detail="Only recommendations frozen before their outcome are included. The result is early until at least 30 trades and mature at 100." /><div className="cfo-summary-strip"><Metric label="Status" value={(backtest.status || 'no_data').replaceAll('_', ' ')} note={`${validation.sample || 0} resolved trades`} /><Metric label="Net expectancy" value={`${fmt(validation.net_expectancy_r, 3)}R`} note={`${fmt(validation.gross_expectancy_r, 3)}R before costs`} /><Metric label="Net result" value={`${fmt(validation.net_total_r, 2)}R`} /><Metric label="Max drawdown" value={`${fmt(validation.max_drawdown_r, 2)}R`} /><Metric label="Win rate" value={`${fmt(validation.win_rate_pct)}%`} note={validation.win_rate_95ci_pct ? `95% range ${validation.win_rate_95ci_pct[0]}–${validation.win_rate_95ci_pct[1]}%` : '95% range unavailable'} /><Metric label="Round-trip costs" value={`${fmt(backtest.cost_model?.round_trip_bps, 0)} bps`} /></div><p className="cfo-footnote">Forward snapshot replay · conservative daily-bar fills · corporate-action-adjusted levels. Historical constituent masters before this ledger began cannot be reconstructed.</p></section>
    <section className="cfo-panel"><SectionHeader eyebrow="Shadow testing" title="Production, V2, and human judgment" detail="A challenger is calibrated only after enough clean outcomes exist, and it cannot replace production automatically." /><div className="cfo-shadow-grid"><article><StatusPill value="healthy">Production</StatusPill><h3>Current algorithm</h3><strong>{brief.model_version || shadow.current?.model_version || 'Current rules'}</strong><p>{validation.sample || 0} resolved outcomes · live decisions remain unchanged.</p></article><article><StatusPill value={shadow.challenger?.status === 'eligible_for_calibration' ? 'attention' : 'neutral'}>{(shadow.challenger?.status || 'awaiting_evidence').replaceAll('_', ' ')}</StatusPill><h3>V2 challenger</h3><strong>{promotion.remaining ?? 100} outcomes remaining</strong><p>{promotion.next_step || 'Continue forward collection without changing live weights.'}</p></article><article><StatusPill value="neutral">Human review</StatusPill><h3>Structured judgment</h3><strong>{reviews.total || 0} reviews</strong><p>Agreement and disagreement stay linked to the exact model snapshot.</p></article></div></section>
  </div>;
}
