...
function System({ job, brief }) {
  ...
  return <div className="cfo-page"><SectionHeader eyebrow="System" title="Data, automation and audit" detail="The workspace prepares itself. Job controls stay here, away from daily decisions." />
    ...
    <section className="cfo-panel"><SectionHeader eyebrow="Historical truth" title="Automatic recommendation outcomes" detail="Every ready-now or near-entry recommendation is followed from its original snapshot. Exclusions remain separate from resolved outcomes." /><div className="cfo-summary-strip"><Metric label="Tracked" value={truth.total || 0} note={`${truth.armed || 0} waiting · ${truth.active || 0} active`} /><Metric label="Resolved" value={truth.resolved || 0} note={`${truth.excluded || 0} excluded`} /><Metric label="Win rate" value={`${fmt(truth.win_rate_pct)}%`} /><Metric label="Expectancy" value={`${fmt(truth.expectancy_r, 2)}R`} /><Metric label="Average MFE" value={`${fmt(truth.avg_mfe_r, 2)}R`} /><Metric label="Average MAE" value={`${fmt(truth.avg_mae_r, 2)}R`} /></div></section>
